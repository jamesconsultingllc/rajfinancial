...
public class TestUserCleanupHelper
{
    ...
    public async Task<int> DeleteAllTestUsers()
    {
        ...
        var filter = $"startswith(mail,'{TestUserEmailPattern}') or startswith(userPrincipalName,'{TestUserEmailPattern}')";
        var response = await httpClient.GetAsync($"users?$filter={Uri.EscapeDataString(filter)}&$select=id,mail,userPrincipalName");

        if (!response.IsSuccessStatusCode)
        {
            throw new Exception($"Failed to query users: {response.StatusCode}");
        }
        ...
        foreach (var user in users.EnumerateArray())
        {
            ...
            var email = user.TryGetProperty("mail", out var mailProp) ? mailProp.GetString() :
    ...
    private async Task<string> GetAccessToken()
    {
        ...
        if (!string.IsNullOrEmpty(clientSecret) && !string.IsNullOrEmpty(tenantId) && !string.IsNullOrEmpty(clientId))
        {
            ...
            credential = new Azure.Identity.ClientSecretCredential(tenantId, clientId, clientSecret);
        }
        else
        {
