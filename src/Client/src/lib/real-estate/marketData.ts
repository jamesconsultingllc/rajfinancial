...
// State-level property tax rates (effective rates as % of assessed value)
...
// State-level homeowners insurance rates (annual as % of home value)
const stateInsuranceRates: Record<string, number> = {
  AL: 0.60, AK: 0.44, AZ: 0.38, AR: 0.65, CA: 0.40,
  CO: 0.50, CT: 0.42, DE: 0.35, FL: 0.75, GA: 0.48,
  HI: 0.25, ID: 0.38, IL: 0.45, IN: 0.42, IA: 0.45,
  KS: 0.60, KY: 0.52, LA: 0.72, ME: 0.38, MD: 0.38,
  MA: 0.42, MI: 0.45, MN: 0.48, MS: 0.68, MO: 0.55,
  MT: 0.50, NE: 0.55, NV: 0.35, NH: 0.38, NJ: 0.42,
  NM: 0.45, NY: 0.42, NC: 0.45, ND: 0.52, OH: 0.40,
  OK: 0.65, OR: 0.35, PA: 0.38, RI: 0.48, SC: 0.52,
  SD: 0.50, TN: 0.52, TX: 0.62, UT: 0.35, VT: 0.40,
  VA: 0.38, WA: 0.38, WV: 0.45, WI: 0.42, WY: 0.45,
  DC: 0.32,
};

// Regional renovation cost multipliers
const regionCosts: Record<string, { light: number; medium: number; heavy: number }> = {
  northeast: { light: 30, medium: 65, heavy: 130 },
  southeast: { light: 20, medium: 45, heavy: 95 },
  midwest: { light: 18, medium: 40, heavy: 85 },
  southwest: { light: 22, medium: 50, heavy: 105 },
  west: { light: 35, medium: 70, heavy: 140 },
  pacific: { light: 38, medium: 75, heavy: 150 },
};
...
const zipToState: Record<string, string> = {
  ...
  '640': 'KS', '641': 'MO', '644': 'MO', '645': 'MO', '646': 'MO', '647': 'MO', '648': 'MO', '649': 'MO', '650': 'MO', '651': 'MO', '652': 'MO', '653': 'MO', '654': 'MO', '655': 'MO', '656': 'MO', '657': 'MO', '658': 'MO', '659': 'MO',
  ...
  '960': 'CA', '961': 'CA',
  '962': 'AP', '963': 'HI', '964': 'HI', '965': 'HI', '966': 'HI', '967': 'HI', '968': 'HI',
...
const stateToRegion: Record<string, string> = {
  ...
  DE: 'southeast', FL: 'southeast', GA: 'southeast', MD: 'southeast', NC: 'southeast', SC: 'southeast', VA: 'southeast', DC: 'southeast', WV: 'southeast',
  AL: 'southeast', KY: 'southeast', MS: 'southeast', TN: 'southeast',
  AR: 'southeast', LA: 'southeast', OK: 'southwest', TX: 'southwest',
  AZ: 'southwest', CO: 'southwest', ID: 'southwest', MT: 'southwest', NV: 'southwest', NM: 'southwest', UT: 'southwest', WY: 'southwest',
  AK: 'pacific', CA: 'pacific', HI: 'pacific', OR: 'west', WA: 'west',
};

export function getStateFromZip(zip: string): string {
  const prefix = zip.substring(0, 3);
  return zipToState[prefix] || 'TX';
}

export function getMarketData(zip: string): MarketData {
  const state = getStateFromZip(zip);
  const region = stateToRegion[state] || 'southeast';
  const taxRate = stateTaxRates[state] || 1.0;
  const insuranceRate = stateInsuranceRates[state] || 0.45;
  const renovationCostPerSqft = regionCosts[region] || regionCosts.southeast;

  // Estimate median price and rent-to-price ratio based on region
  const regionDefaults: Record<string, { medianPrice: number; rentToPrice: number; appreciation: number }> = {
    northeast: { medianPrice: 420000, rentToPrice: 0.55, appreciation: 3.2 },
    southeast: { medianPrice: 340000, rentToPrice: 0.65, appreciation: 4.0 },
    midwest: { medianPrice: 260000, rentToPrice: 0.72, appreciation: 3.5 },
    southwest: { medianPrice: 380000, rentToPrice: 0.58, appreciation: 3.8 },
    west: { medianPrice: 520000, rentToPrice: 0.48, appreciation: 3.5 },
    pacific: { medianPrice: 680000, rentToPrice: 0.42, appreciation: 3.0 },
  };

  const defaults = regionDefaults[region] || regionDefaults.southeast;

  return {
    taxRate,
    insuranceRate,
    medianPrice: defaults.medianPrice,
    rentToPrice: defaults.rentToPrice,
    renovationCostPerSqft,
    appreciationRate: defaults.appreciation,
    region: `${state} (${region.charAt(0).toUpperCase() + region.slice(1)})`,
  };
}
